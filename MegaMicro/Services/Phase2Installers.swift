...
final class OpenCodeIntegration: ProviderIntegration {
    ...
    init(pluginURL: URL = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".config/opencode/plugins/megamicro.ts"), port: UInt16) {
        self.pluginURL = pluginURL; self.port = port
    }
    func isInstalled() -> Bool { FileManager.default.fileExists(atPath: pluginURL.path) }
    func install() throws {
        let source = """
        // MegaMicro lifecycle telemetry. This file is owned by MegaMicro.
        import type { Plugin } from "@opencode-ai/plugin"

        export const MegaMicro: Plugin = async ({ directory }) => ({
          event: async ({ event }) => {
            const e: any = event
...
final class CursorIntegration: ProviderIntegration {
    ...
    var isDetected: Bool {
        executableExists(["agent", "cursor-agent", "cursor"])
            || NSWorkspace.shared.urlForApplication(withBundleIdentifier: DefaultProfiles.cursorBundleID) != nil
    ...
    func isInstalled() -> Bool {
        ...
        return hooks.values.contains { String(describing: $0).contains("#megamicro") }
    }
    ...
    private func removeOwned(from hooks: inout [String: Any]) {
        for (event, value) in hooks {
            ...
            let kept = entries.filter { !String(describing: $0).contains("#megamicro") }
...
final class QwenIntegration: ProviderIntegration {
    ...
    init(settingsURL: URL = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".qwen/settings.json"),
         backupDir: URL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0].appendingPathComponent("MegaMicro/backups"),
         port: UInt16, bridgeURL: URL? = nil) {
